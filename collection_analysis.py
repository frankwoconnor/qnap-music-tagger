...
def analyze_and_suggest_tag_rules(
    ...
    standardized_genres_list = sorted(list(set(standardized_genres_list)))
...
def save_genre_rules_to_yaml(
    ...
    minimal_genres_list: List[str],
    yaml_path: str = "tag_rules.yaml"
):
    """Saves genre standardization rules and minimal genre mapping to tag_rules.yaml."""
    try:
...
def load_genre_rules_from_yaml(yaml_path: str = "tag_rules.yaml") -> Dict[str, Any]:
    """Loads genre standardization rules and minimal genre mapping from tag_rules.yaml."""
    try:
        with open(yaml_path, 'r', encoding='utf-8') as f:
            config = yaml.safe_load(f)
...
def generate_minimal_genre_mapping_suggestions(
    ...
    """Generate minimal genre mapping suggestions using keyword matching and MusicBrainz."""
